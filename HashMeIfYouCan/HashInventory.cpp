...
Inventory<Comparator, std::unordered_set<Item>>::Inventory() : items_{}, equipped_{nullptr}, weight_{0.0f}
{
    // your code here
}
...
{
    // your code here
    Item delByName{itemName, 0.0f, NONE};
    auto it = items_.find(delByName);
    if (it != items_.end())
    {
        weight_ -= it->weight_;
        items_.erase(it);
...
{
    // your code here
    return items_.find(Item{itemName, 0.0f, NONE}) != items_.end(); // constant time lookup O(1)
}
