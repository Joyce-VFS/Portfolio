...
public:
    ...
    void setCuisineType(const CuisineType& cuisine_type);
     ...
     //Adding here the equality operator overload (==) calling the right-hand-side(rhs) object passed as argument
    /**
     @param : A const reference to the right-hand side of the `==` operator.
     @return : Returns true if the right-hand side dish is "equal", false otherwise.
     Two dishes are equal if they have the same name, same cuisine type, same preparation time, and the same price.
     */
    bool operator==(const Dish& rhs) const;

     //Adding here the inequality operator overload (!=) calling the right-hand-side(rhs) object passed as argument
    /**
     @param : A const reference to the right-hand side of the `!=` operator.
